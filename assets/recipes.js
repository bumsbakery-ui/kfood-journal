(() => {
  ...
  const canonicalAliases = new Set(['naengmyeon-kr', 'galbijjim-kr']);
  let posts = [];
  const decode = (value) => { const el=document.createElement('textarea'); el.innerHTML=value; return el.value; };
  const detailUrl = (post) => {
    try { return new URL(post.url).pathname; }
    catch { return `/${post.slug}/`; }
  };
  const render = (items) => {
  ...
  fetch('/assets/data/posts.json?v=20260721-static').then(r=>r.json()).then(data=>{ posts=data.filter(post=>post.language===lang&&!canonicalAliases.has(post.slug)); render(posts); document.querySelector('[data-recipe-count]').textContent=posts.length; }).catch(()=>{ list.innerHTML=`<p class="empty-state">${lang==='ko'?'레시피를 불러오지 못했습니다.':'Recipes could not be loaded.'}</p>`; });
