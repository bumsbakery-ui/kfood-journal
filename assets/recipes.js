(() => {
  ...
  fetch('/assets/data/posts.json?v=20260720-naengmyeon').then(r=>r.json()).then(data=>{ posts=data.filter(post=>post.language===lang); render(posts); document.querySelector('[data-recipe-count]').textContent=posts.length; }).catch(()=>{ list.innerHTML=`<p class="empty-state">${lang==='ko'?'레시피를 불러오지 못했습니다.':'Recipes could not be loaded.'}</p>`; });
